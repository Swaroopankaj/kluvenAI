function extractBaseAgentKey(uniqueId: string): string {
  return uniqueId.replace(/_[a-z0-9]{6}$/, '');
}
...
export function createAgentDisplayNames(agentIds: string[]): Map<string, string> {
  ...
  agentIds.forEach(agentId => {
    ...
    const readableName = baseKey.replace(/_/g, ' ').replace(/\b\w/g, (l: string) => l.toUpperCase());
